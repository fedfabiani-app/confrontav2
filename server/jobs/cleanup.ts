 ...
 */
export async function cleanupOldHoroscopeData(): Promise<{ deleted: number }> {
  ...
  try {
    ...
    // Delete old records
    const deleteResult = await prisma.horoscopeData.deleteMany({
      where: {
        date: {
          lt: cutoffDate
        }
    ...
    console.log(`[Cleanup] Deleted ${deleteResult.count} old horoscope records`);
    
    // Reset the ID sequence if any records were deleted
    if (deleteResult.count > 0) {
      ...
      console.log(`[Cleanup] Reset ID sequence to start at ${nextId}`);
    }
