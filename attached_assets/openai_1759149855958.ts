...
export async function extractHoroscopeData(
  client: OpenAI,
  text: string,
  zodiacSign: string
): Promise<{
  ...
  tone_analysis: 'positive' | 'negative' | 'neutral';
}> {
  // Validate input text quality
  if (text.length < 50) {
    throw new Error('Input text too short for meaningful analysis');
  }

  // Check if text seems to be actual horoscope content - expanded keyword list
  const hasHoroscopeKeywords = /\b(oroscopo|previsioni|stelle|fortuna|amore|lavoro|salute|giornata|periodo|energia|voto|destino|luna|sole|pianeti|segno|zodiaco|oggi|domani|settimana|relazioni|carriera|benessere|marte|venere|saturno|giove|mercurio|plutone|nettuno|urano|ariete|toro|gemelli|cancro|leone|vergine|bilancia|scorpione|sagittario|capricorno|acquario|pesci|ascendente|dominante|leader|naturale|entusiasmo|decisioni|timore|pazienza|ottimale|opposizione|sbilanciamento|comunicazione|magnetica|presenza|qualità|valorizza|godervi|vita|passione|romanticismo|fantasia|cuore|eros|disinvoltura|ufficio|competenza|equilibrio|forma|fisica|pimpanti|questione|risoluto|sbrigativo|garbo|cielo|aiuta)\b/i.test(text);
  ...
  // Check for astrological content patterns - be more inclusive
  const hasAstrologicalContent = /\b(pianeta|congiunzione|quadratura|trigono|sestile|casa|cuspide|transiti?|aspetti?|influssi?|influenze?|configurazioni?|astri|cielo|combinazione|astrologica)\b/i.test(text);
  const hasPersonalityTraits = /\b(leader|naturale|entusiasmo|contagioso|decisioni|rapide|magnetica|presenza|qualità|carattere|personalità|temperamento|grinta|tenero|affettuoso|piglio)\b/i.test(text);
  const hasEmotionalContent = /\b(timore|pazienza|energia|vitalità|serenità|nervosismo|ansia|felicità|tristezza|preoccupazioni|ottimismo|passione|romanticismo|fantasia|tenerezza|connessione|emotiva|volontà|disinvoltura)\b/i.test(text);
  
  // Check for typical horoscope action words and phrases
  const hasActionWords = /\b(spunta|mettete|posto|gestire|aiuta|godere|vivere|ascoltare|prestare|attenzione|concedetevi|relax|sarà|momento|positivo|risolverle|discussioni|avete|ragione|non|avete|paura|accettare|sorprese|migliora|giorno|dopo|giorno)\b/i.test(text);
  
  // Be more lenient for content that might be horoscope-related
  const hasAnyRelevantContent = hasHoroscopeKeywords || hasAstrologicalContent || hasPersonalityTraits || hasEmotionalContent || hasActionWords;
  
  if (!hasAnyRelevantContent || (hasNavigationWords && text.length < 200) || (hasPaywallWords && text.length < 150)) {
    console.log('OpenAI validation failed for text:', text.substring(0, 200));
    console.log('Has horoscope keywords:', hasHoroscopeKeywords);
    console.log('Has astrological content:', hasAstrologicalContent);
    console.log('Has personality traits:', hasPersonalityTraits);
    console.log('Has emotional content:', hasEmotionalContent);
    console.log('Has action words:', hasActionWords);
    console.log('Has navigation words:', hasNavigationWords);
    console.log('Has paywall words:', hasPaywallWords);
    throw new Error('Text does not appear to contain horoscope content');
  }
    ...
    response_format: {
      ...
      json_schema: {
        ...
        schema: {
          ...
          required: ['summary', 'relazioni_rating', 'lavoro_rating', 'salute_rating', 'tone_analysis'],
